...
// The interactive core of sign-in — shared between the standalone /login
// page (kept as a no-JS/bookmark fallback) and the landing page's morphing
// sign-in popover, so the two never drift out of sync. Each caller supplies
// its own heading/chrome around this.
export function SignInFields({ next = "/hub" }: { next?: string }) {
  ...
  async function sendCode(e: React.FormEvent) {
    ...
    // Gate code requests to emails we already know about — otherwise
    // anyone could type in an arbitrary address, get an account
    // auto-created (the profiles trigger accepts any sign-in), and see
    // more of the site than they should. This check runs server-side
    // against the guest list, which the browser has no read access to.
    const eligibility = await fetch("/api/auth/request-code", {
      method: "POST",
    ...
    if (!eligibility.ok) {
      ...
      setError(
        body?.error ??
          "We don't have that email on our guest list — check with Nick or Ellie."
      );
      return;
    }
  ...
  return (
    <div className="w-full text-center">
      <div className="mx-auto flex max-w-[220px] gap-0.5 rounded-full bg-cream-deep p-1">
        <button
          type="button"
